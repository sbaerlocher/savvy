<script lang="ts">
	...
	// Bindable: the page renders the iOS create button on the title row and
	// needs the config gate this section already loads.
	let { localLoginEnabled = $bindable(true) }: { localLoginEnabled?: boolean } =
		$props();
...
{:else if IOS}
						...
						>
							...
							<div class="flex flex-wrap gap-2">
								...
								<button
									type="button"
									onclick={() => toggleRole(user.id, user.role)}
									disabled={isOffline || user.id === currentUser?.id || isOAuth}
	...
	{/if}
{:else if IS_ANDROID}
	<!-- Create user as the screen's FAB (M3: one FAB per screen). MobileNav
