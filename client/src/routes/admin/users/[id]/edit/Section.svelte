				...
				>
					...
					<div>
						...
						<input
							...
							required
							disabled={!isAdmin}
							class="input disabled:bg-border-soft disabled:cursor-not-allowed"
						/>
					</div>
					...
					<div>
						<label for="user-provider" class="label">
					...
					{#if isOffline}
						<p class="text-sm text-warning-600 text-center">
				...
				{#if !isEditingSelf}
					<div class="pt-6 mt-6 border-t border-border">
						...
						<button
							...
							class="btn btn-sm btn-primary w-full {IS_ANDROID
								? 'rounded-m3-full'
								: ''}"
						>
						...
						{#if isOffline}
							<p class="text-sm text-warning-600 text-center mt-2">
								{$t('common.offlineEditDisabled')}
							</p>
						{/if}
					</div>
