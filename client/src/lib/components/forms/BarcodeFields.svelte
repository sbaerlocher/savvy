<script lang="ts">
	...
	interface Props {
		...
		inputId?: string;
		placeholder?: string;
		/** Render only one half. The desktop edit forms pair the type select with
		 *  a neighbouring field (mockup), so they place the two halves themselves;
		 *  'both' (default) keeps the stacked pair this component has always
		 *  emitted. */
		part?: 'both' | 'value' | 'type';
	}
