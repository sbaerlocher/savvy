...
>
	{#if platform === 'ios'}
		...
		<div class="flex items-center justify-between px-4 py-2">
			...
			<span class="text-[length:var(--text-label)] font-medium text-text-subtle tabular-nums">
				{selectedCount} / {totalCount}
			</span>
			<button
				type="button"
		...
		>
			...
			>
				...
				<span class="text-[length:var(--text-tag)]">{tr('common.transferOwnership')}</span>
			</button>
			<button
				type="button"
				onclick={onExport}
	...
	{:else}
		...
		<div class="flex items-center justify-evenly px-2 py-2">
			...
			>
				...
				<span class="text-[length:var(--text-tag)] font-medium">{tr('common.share')}</span>
			</button>
			...
			>
				...
				<span class="text-[length:var(--text-tag)] font-medium">{tr('common.delete')}</span>
			</button>
		</div>
	{/if}
</div>
