...
/**
 * Create lazy-loading wrapper for scanner functions
 * @param {Function} scannerFactory - Original scanner factory function
 * @returns {Function} Wrapped factory with lazy loading
 */
function createLazyScannerWrapper(scannerFactory) {
	return function(...args) {
		const scannerInstance = scannerFactory(...args);
		const originalStartScanning = scannerInstance.startScanning;

		// Override startScanning to load module first
		scannerInstance.startScanning = async function() {
			this.scanning = true;
			this.scanMessage = 'Barcode-Scanner wird geladen...';

			try {
				// Load scanner module on first scan
				await loadScannerModule();
				this.scanMessage = 'Kamera wird gestartet...';

				// Call original startScanning
				return await originalStartScanning.call(this);
			} catch (error) {
				console.error('Failed to load scanner module:', error);
				this.scanMessage = 'Scanner konnte nicht geladen werden';
