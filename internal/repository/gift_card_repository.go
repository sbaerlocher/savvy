...
type GiftCardRepository interface {
	...
	GetAllForUserPaginated(ctx context.Context, userID uuid.UUID, params PaginationParams) (*PaginatedResult[models.GiftCard], error)

	// GetExpiringGiftCards retrieves gift cards expiring within the given number of days
	GetExpiringGiftCards(ctx context.Context, withinDays int) ([]models.GiftCard, error)
