...
type ShareAdapter interface {
	...
	// DeleteShare removes a share identified by the shared user and resource
	DeleteShare(ctx context.Context, callerUserID, sharedWithID, resourceID uuid.UUID) error
...
type UpdateShareRequest struct {
	CallerUserID        uuid.UUID // User requesting update (must be owner)
	SharedWithID        uuid.UUID // User whose share is being updated
