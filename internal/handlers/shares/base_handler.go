...
func (h *BaseShareHandler) Create(c *echo.Context) error {
	...
	isHTMX := c.Request().Header.Get("HX-Request") == htmxTrue
	...
	if isHTMX {
		c.Response().Header().Set("HX-Refresh", htmxTrue)
...
func (h *BaseShareHandler) Update(c *echo.Context) error {
	...
	sharedWithUUID, err := uuid.Parse(sharedWithID)
	if err != nil {
	...
	// HTMX: refresh page
	if c.Request().Header.Get("HX-Request") == htmxTrue {
		c.Response().Header().Set("HX-Refresh", htmxTrue)
		return c.String(http.StatusOK, "")
	}
...
func (h *BaseShareHandler) Delete(c *echo.Context) error {
	...
	// Delete using adapter (delegates to ShareService for ownership + scoping)
	if err := h.adapter.DeleteShare(ctx, user.ID, sharedWithUUID, resourceUUID); err != nil {
