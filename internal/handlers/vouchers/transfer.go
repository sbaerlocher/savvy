// Package vouchers handles voucher-related HTTP requests.
package vouchers
...
func (h *Handler) TransferInline(c echo.Context) error {
	...
	return templates.VoucherTransferInlineForm(c.Request().Context(), csrfToken, voucherID.String()).
...
func (h *Handler) Transfer(c echo.Context) error {
	...
	if err := h.transferService.TransferVoucherOwnership(c.Request().Context(), voucherID, newOwner.ID, user.ID); err != nil {
	...
	// HTMX: Redirect to vouchers list (user no longer owns this voucher)
